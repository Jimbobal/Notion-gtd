/* Calendar — the hard landscape. Your own dated items (calendar entries,
   deadlines, chase-by dates) together with Google and Outlook events, day
   by day. Events open in a sheet where they can be edited, deleted or
   turned into a GTD item; new events can be created from any day. */
'use strict';
...
export function render() {
  ...
  const addButtons = day => `<button class="link-btn" data-act="cal-add" data-v="${day}">+ Item</button>${writableTargets().length ? ` <button class="link-btn" data-act="cal-add-event" data-v="${day}">+ Event</button>` : ''}`;
  ...
  if (sel) {
    ...
    const laterEv = visibleEvents().filter(e => e.day > horizon);
...
function eventForm(e = {}, o = {}) {
  ...
  return `<h3>${e.id ? 'Edit event' : 'New event'}</h3>
    <form id="gevent-form" data-id="${attr(e.eventId || '')}" data-cal="${attr(e.calendarId || '')}" data-kind="${attr(e.kind || '')}">
      ...
      ${e.id ? '' : field('Calendar', `<select name="target">${options(targets.map(t => [t.value, t.label]), o.target || targets[0]?.value || '')}</select>`)}
...
function openEvent(id) {
  ...
  openSheet(`${writable ? eventForm(e) : `<h3>${esc(e.title)}</h3><p class="note">${esc(src?.name || '')} is read only.</p>`}
    <div class="sheet-actions plain">
      ...
      ${e.htmlLink ? `<a class="btn" href="${attr(e.htmlLink)}" target="_blank" rel="noopener">Open in ${esc(src?.provider || 'calendar')}</a>` : ''}
    </div>`);
}
...
export async function act(name, el) {
  ...
  switch (name) {
    ...
    case 'cal-add-event': openSheet(eventForm({}, { day: v })); return true;
    case 'cal-source': { const s = sourceById(id); toggleSource(id); if (s) document.dispatchEvent(new CustomEvent('gtd:calendar', { detail: { provider: s.kind } })); return 'render'; }
    ...
    case 'ev-delete': {
      ...
      if (!confirm(`Delete this event from ${sourceById(e.feedId)?.provider || 'the calendar'}?`)) return true;
      closeSheet();
      try { await PROVIDERS[e.kind].remove(e.calendarId, e.eventId); toast('Event deleted'); await refreshProvider(e.kind); }
    ...
    case 'ev-to-item': {
      ...
      await A.addItem({ name: e.title, status: 'Calendar', date, time: mins, notes: [e.location, e.description].filter(Boolean).join('\n'),
                        eventId: `${e.kind}|${e.calendarId}|${e.eventId}` });
      await refreshProvider(e.kind).catch(() => {});
      return 'render';
    }
  }
  return false;
...
export async function submit(form) {
  ...
  const fields = { title: f.title.trim(), day: f.day, time: f.allDay ? '' : f.time, endTime: f.allDay ? '' : f.endTime,
                   allDay: !!f.allDay || !f.time, location: f.location.trim(), description: f.description.trim() };
  const target = form.dataset.id ? { kind: form.dataset.kind, calendarId: form.dataset.cal } : parseTarget(f.target);
  if (!target) return toast('Choose a calendar.'), true;
  const p = PROVIDERS[target.kind];
  closeSheet();
  try {
    if (form.dataset.id) { await p.update(target.calendarId, form.dataset.id, p.body(fields)); toast('Event saved ✓'); }
    else { await p.create(target.calendarId, p.body(fields)); toast(`Event created in ${p.label} ✓`); }
    await refreshProvider(target.kind);
